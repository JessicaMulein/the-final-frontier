...
def _report_preflight(plan: FrozenBatchPlan):
    targeted = TargetedCheckBinding(
        result_path="audiobook-studio/build/targeted-checks.json",
        result_byte_count=2,
    ...
    allowlist = _sealed(
        WorkflowWriteAllowlist(
            recursive_roots=(
                f"audiobook-studio/build/production/{plan.book_id}",
            ),
    ...
    filesystem = _sealed(
        FilesystemCheck(
            ...
            checked_devices=(("audiobook-studio", 1),),
            blocking_categories=(),
            canonical_sha256="",
        )
    )
...
def _accounting(rate_card: OfficialRateCard):
    ...
    aggregation = JournalAggregation(
        journals=(
            ResolvedJournalUsage(
                segment_id="segment-001",
                audio_path="audiobook-studio/build/runtime/segment-001.wav",
                journal_path="audiobook-studio/build/runtime/segment-001.events.jsonl",
...
def test_deliver_operation_creates_complete_sanitized_idempotent_track_and_batch_reports(
    ...
    batch_path = (
        workspace
        / "audiobook-studio/build/production/the-final-frontier/batches/plan-delivery/report.md"
    )
