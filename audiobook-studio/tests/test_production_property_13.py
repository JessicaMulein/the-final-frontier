...
def _write_protected_tree(
    ...
    runtime = _write_bytes(
        workspace / "audiobook-studio/src/frontier_audiobook/runtime_guard.py",
        f'RUNTIME_MARKER = "case-{case_index:03d}"\n'.encode("utf-8"),
    )
    dependency_paths = (
        _write_bytes(
            workspace / "audiobook-studio/pyproject.toml",
            f"[project]\nname = \"fixture-{case_index:03d}\"\n".encode("utf-8"),
        ),
        _write_bytes(workspace / "audiobook-studio/uv.lock", rng.randbytes(48)),
        _write_bytes(workspace / "audiobook-studio/requirements/dev.txt", b"pytest==9.1.1\n"),
        _write_bytes(workspace / "audiobook-studio/requirements/runtime.txt", b"runtime==1.0.0\n"),
    )
    ...
    for chapter in (1, 2):
        ...
        legacy_root = (
            workspace
            / "audiobook-studio/build/narration"
...
def test_property_13_protected_isolation_is_per_file_and_attributable(tmp_path):
    ...
    for case_index in range(GENERATED_CASES):
        ...
        if case.include_outside_workflow_write:
            outside_write = f"audiobook-studio/build/unapproved/property-13-{case_index:03d}.json"
            _write_bytes(
