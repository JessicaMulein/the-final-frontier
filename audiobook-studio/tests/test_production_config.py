...
def test_tracked_config_has_required_defaults_and_pending_render_once_catalog():
    ...
    config_path = workspace_root / "audiobook-studio" / "config" / "production.toml"
...
def test_waveform_lexicon_hyphenates_without_touching_neighbor_words():
    workspace_root = Path(__file__).resolve().parents[2]
    config = load_production_config(workspace_root / "audiobook-studio" / "config" / "production.toml")
    source = (
