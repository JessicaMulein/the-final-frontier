...
def read_chapter(path: Path, *, enforce_declared_words: bool = True) -> ChapterSource:
    ...
    try:
        title = json.loads(header["title"])
    except (json.JSONDecodeError, ValueError) as exc:
        raise InputError(
            f"Chapter title must be one complete double-quoted string in {path}"
        ) from exc
    if (
        ...
        raise InputError(f"Chapter title must be nonblank single-line plain text in {path}")
