...
def _add_production_parser(commands: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    plan.add_argument(
        "--config",
        type=Path,
        help="Production TOML; defaults to audiobook-studio/config/production.toml",
    )
...
def _parser() -> argparse.ArgumentParser:
    ...
    plan.add_argument("--config", type=Path, help="Audition TOML; defaults to audiobook-studio/config/audition.toml")
...
def _workspace_root(value: Path | None) -> Path:
    if value is not None:
        ...
        config_root = root / "audiobook-studio" / "config"
        if not (
    ...
    for candidate in (current, *current.parents):
        config_root = candidate / "audiobook-studio" / "config"
        if (
            (config_root / "audition.toml").is_file()
            or (config_root / "production.toml").is_file()
        ):
...
def _production_config_path(workspace: Path, value: Path | None = None) -> Path:
    return (
        ...
        if value is not None
        else resolve_inside(workspace, "audiobook-studio/config/production.toml")
    )
...
def run(arguments: argparse.Namespace) -> int:
    ...
    if arguments.phase == "plan":
        config_path = (
            ...
            if arguments.config
            else workspace / "audiobook-studio" / "config" / "audition.toml"
        )
    ...
    if arguments.phase == "narrate":
        config = load_audition_config(
            workspace / "audiobook-studio" / "config" / "audition.toml",
            workspace,
        )
        if arguments.dry_run:
