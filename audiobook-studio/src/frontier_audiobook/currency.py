...
Production manifests record both the whole source file and the normalized prose body.
For chapter audio, currency prefers ``normalized_body_sha256`` so restricted-header
metadata edits (including canonical chapter titles) require metadata/announcement
rebuilds but do not falsely require a narration re-render. Older manifests that lack
the body digest retain their historical raw-byte convention. Legacy narration already
records the prose-body digest directly.
...
class AudioBinding:
    ...
    source_digest_convention: SourceDigestConvention
    audio_path: str
...
def discover_production_bindings(
    workspace_root: Path,
    *,
    build_root: str = "audiobook-studio/build/production",
) -> tuple[AudioBinding, ...]:
    ...
    for delivery_path in sorted(root.glob("*/transactions/*/*/delivery/delivery.json")):
        ...
        recorded_raw_source = _digest(manifest.get("source_sha256"))
        recorded_body_source = _digest(manifest.get("normalized_body_sha256"))
        if recorded_body_source is not None and _chapter_number(track_id) is not None:
            recorded_source = recorded_body_source
            source_convention = SourceDigestConvention.PROSE_BODY
        else:
            recorded_source = recorded_raw_source
            source_convention = SourceDigestConvention.RAW_BYTES
        ...
        binding = AudioBinding(
            ...
            source_digest_convention=source_convention,
...
def discover_legacy_bindings(
    workspace_root: Path,
    *,
    narration_root: str = "audiobook-studio/build/narration",
) -> tuple[AudioBinding, ...]:
    ...
    for manifest_path in sorted(root.glob("*/manifest.json")):
        ...
        bindings.append(
            AudioBinding(
                ...
                source_path=source_path,
                recorded_source_sha256=recorded_source,
                source_digest_convention=SourceDigestConvention.PROSE_BODY,
                audio_path=audio_path,
                recorded_audio_sha256=recorded_audio,
            )
        )
...
def evaluate_binding(workspace_root: Path, binding: AudioBinding) -> CurrencyResult:
    ...
    convention = binding.source_digest_convention
