...
Execute the existing Chapter 2 narration workflow without changing code or prose. Each leaf task produces or verifies an operational artifact under `audiobook-studio/build/narration/chapter-002-tiffany/`; the single paid task is isolated between preflight and postflight waves. Stop on any failed hard gate—never retry a failed or charge-uncertain model call automatically.
...
- [ ] 1. Complete nonbillable preflight
  - [ ] 1.1 Inspect and freeze the local execution contract
    ...
    - Record clean pre-execution status/hashes for protected prose/spec/tooling/dependency paths, all `chapter-001-*` build trees, and `voice-samples/chapter-1-*` in `audiobook-studio/build/narration/chapter-002-tiffany/evidence/preflight-local.json`.
    - _Requirements: 1.1, 1.2, 1.5, 1.7, 7.8, 7.9, 7.10_

  - [ ] 1.2 Run and record the offline Chapter 2 dry-run
    - From `audiobook-studio/`, run `.venv/bin/python -m frontier_audiobook audition narrate --chapter 2 --voice tiffany --max-words 5 --dry-run` without an AWS profile requirement or paid confirmation.
    - Save the JSON output as `audiobook-studio/build/narration/chapter-002-tiffany/evidence/preflight-dry-run.json`; verify chapter `2`, voice `tiffany`, zero billable calls, positive planned calls, and the expected source-derived word count.
  ...
  - [ ] 1.3 Verify the external paid boundary and official rates
    ...
    - Read the official Amazon Bedrock `us-east-1` offer for model `Nova Sonic 2.0` and record all four on-demand rates, units, source URL, retrieval/effective dates, model, and region in `audiobook-studio/build/narration/chapter-002-tiffany/evidence/preflight-external.json`.
...
- [ ] 2. Execute the explicitly authorized paid render/resume
  - [ ] 2.1 Run the existing Chapter 2 narration command exactly once
    ...
    - From `audiobook-studio/`, run `AWS_PROFILE=frontier-audiobook .venv/bin/python -m frontier_audiobook audition narrate --chapter 2 --voice tiffany --max-words 5 --confirm-paid-render` exactly once and do not add `--accept-verbatim-prefix`.
    - Capture console output under `audiobook-studio/build/narration/chapter-002-tiffany/evidence/render-console.log` and record final CLI fields plus newly narrated/reused segment IDs in `audiobook-studio/build/narration/chapter-002-tiffany/evidence/render-result.json` without transcript bodies or identity details.
...
- [ ] 3. Validate the completed render and calculate usage
  - [ ] 3.1 Validate Property 1: active manifest set is complete and unambiguous
    ...
    - Write pass/fail facts and hashes, without prose, to `audiobook-studio/build/narration/chapter-002-tiffany/evidence/property-1-active-set.json`; a failure blocks all later waves.
  ...
  - [ ] 3.2 Validate Property 2: active segment fidelity is exact
    ...
    - Write aggregate counts and per-segment pass/hash facts to `audiobook-studio/build/narration/chapter-002-tiffany/evidence/property-2-fidelity.json`.
  ...
  - [ ] 3.3 Validate Property 3: assembly integrity is preserved
    ...
    - Write the path, hash, duration, format, segment count, and pass/fail result to `audiobook-studio/build/narration/chapter-002-tiffany/evidence/property-3-assembly.json`.
    - **Property 3: Assembly integrity is preserved**
    - **Validates: Requirements 4.1, 4.2, 4.3, 4.4**

  - [ ] 3.4 Validate Property 4: reuse is excluded from current execution usage
    - Reconcile `audiobook-studio/build/narration/chapter-002-tiffany/evidence/render-result.json`, preflight artifact state, active segment hashes, and final CLI counters.
    ...
    - Write rendered/reused IDs and counts, hash-preservation result, and pass/fail status to `audiobook-studio/build/narration/chapter-002-tiffany/evidence/property-4-reuse.json`.
  ...
  - [ ] 3.5 Validate Property 5: journal aggregation is exact
    ...
    - Write exact per-segment and aggregate integer totals plus reconciliation status to `audiobook-studio/build/narration/chapter-002-tiffany/evidence/property-5-usage.json`.
  ...
  - [ ] 3.6 Validate Property 7 and calculate both pre-tax service costs
    ...
    - Write formulas, provenance, rates, tokens, subtotals, totals, and status to `audiobook-studio/build/narration/chapter-002-tiffany/evidence/property-7-cost.json`.
...
- [ ] 4. Pass the delivery gate and publish the proof copy
  - [ ] 4.1 Evaluate the complete delivery gate and protected-artifact isolation
    ...
    - Write the all-pass gate decision and changed-path list to `audiobook-studio/build/narration/chapter-002-tiffany/evidence/delivery-gate.json`; any protected change or failed validation stops delivery.
  ...
  - [ ] 4.2 Validate Property 6 while creating the clearly named proof copy
    ...
    - Write source/destination paths, sizes, hashes, action, and pass/fail status to `audiobook-studio/build/narration/chapter-002-tiffany/evidence/property-6-proof-copy.json`.
    - **Property 6: Proof delivery preserves audio**
    - **Validates: Requirements 4.5, 4.6, 4.7**

  - [ ] 4.3 Create the final proof report from validated evidence
    - Create `audiobook-studio/build/narration/chapter-002-tiffany/chapter-002-tiffany-proof-report.md` only after Tasks 4.1 and 4.2 pass.
