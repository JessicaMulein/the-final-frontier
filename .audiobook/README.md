...
The tracked authority is the generic Python subsystem under `.audiobook/src/frontier_audiobook/` plus the single strict book configuration at `.audiobook/config/production.toml`. Routine production generates immutable plans, independent track transactions, validation evidence, WAVs, and reports. It does not generate or edit chapter-specific operational specs.
...
cd ..
CLI=".audiobook/.venv/bin/frontier-audiobook"
```
...
`.audiobook/config/production.toml` is strict UTF-8 TOML. Unknown keys, duplicate logical entries, unsafe paths, unknown enum values, and invalid bounds fail before a plan is written. Effective values resolve deterministically in this order:
...
Normal writes are restricted to the selected plan/transaction roots, rebuildable indexes, and approved delivery destinations. Any workflow-owned write outside that allowlist blocks delivery. Existing Chapter 1/2 build roots matching `.audiobook/build/narration/chapter-001-*` and `chapter-002-*`, and proof WAVs matching `voice-samples/chapter-1-*` and `chapter-2-*`, are observed by path/size/SHA-256 and never migrated, renamed, overwritten, or normalized. The historical and governing spec trees listed above are likewise byte-protected.
