...
def test_installed_wheel_plans_single_range_and_render_once_tracks_in_generic_roots(
    ...
    for payload in (single, ranged, special):
        ...
        assert plan_relative.startswith(
            ".audiobook/build/production/the-final-frontier/plans/"
        )
    ...
    generated_files = [
        ...
        if path.is_file() and ".audiobook/build" in path.as_posix()
    ]
    assert generated_files
    assert all(
        path.startswith(".audiobook/build/production/the-final-frontier/plans/")
...
def test_installed_wheel_execute_uses_one_synthetic_direct_child_without_external_calls(
    ...
    setup_script.write_text(
        textwrap.dedent(
            ...
            targeted_path = workspace / ".audiobook/build/targeted/installed-cli.json"
    ...
    attempts = list(
        (
            workspace
            / ".audiobook/build/production/the-final-frontier/transactions"
