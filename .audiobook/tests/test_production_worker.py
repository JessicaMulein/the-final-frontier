...
AUDITION_CONFIG = REPOSITORY_ROOT / ".audiobook" / "config" / "audition.toml"
SENTENCES = (
...
def _write_workspace(
    ...
    *,
    ...
    runtime_config = workspace / ".audiobook" / "config" / "audition.toml"
    ...
    if track_kind == "chapter":
        ...
        selected_source = workspace / ".audiobook" / "config" / "tracks" / "opening.md"
...
sequence = 1
source_path = ".audiobook/config/tracks/opening.md"
    ...
    def render(text, _voice_id, _settings, *, paid_render_authorized=False, **_kwargs):
        ...
        if calls == 1:
            if drift == "source":
                ...
                runtime_path = tmp_path / ".audiobook" / "config" / "audition.toml"
